<?php // ajax json action
	...
	$query = "SELECT COUNT(*) FROM [pre]recall WHERE `viewed`=0 ORDER BY id LIMIT 1000";
	...
	$query = "SELECT COUNT(*) FROM [pre]event_recall WHERE `viewed`=0 ORDER BY id LIMIT 1000";
			
	$_stmt = $dbh->prepare($query);
	$_arr = $_stmt->execute();
		
		$_res = $_arr->fetchallAssoc();
		$ERquant = (int)$_res[0]['COUNT(*)'];
		
	$data['tasksER'] = $ERquant;
