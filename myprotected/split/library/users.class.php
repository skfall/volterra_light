...
class usersHelp extends BasicHelp
{
		...
		public function getAllUsers($params=array(),$typeCount=false)
		{
			...
			if(!$typeCount)
			{
				...
				$query = "SELECT M.type, M.first_name, M.last_name, M.block, M.created, C.*, C.id AS card_id, M.id,      
						(SELECT name FROM [pre]users_types WHERE id=M.type) as type_name
						FROM [pre]users as M  
						LEFT JOIN osc_user_cards AS C ON C.user_id = M.id 
						WHERE 1 $filter_and 
		...
		{
			$query = "
				SELECT M.*, C.*, C.id as user_card_id FROM [pre]users AS M  
				LEFT JOIN `osc_user_cards` AS C ON C.user_id = M.id 
				WHERE M.id=$id 
				LIMIT 1
			";
